...
if uploaded_file is not None:
    ...
    if len(df_p1) == 0 and len(df_p2) == 0:
        ...
        def create_grundydelser_line_chart():
            ...
            # Total antal - P1 (blå)
            fig.add_trace(
                go.Scatter(x=months_p1, y=total_p1, name="Total P1", 
                          line=dict(color='#4169E1', width=3), mode='lines+markers'),
                secondary_y=False
            )
            
            # Total antal - P2 (lyseblå)
            fig.add_trace(
                go.Scatter(x=months_p2, y=total_p2, name="Total P2", 
            ...
            # Procent 120 - P1 (rød)
            fig.add_trace(
                go.Scatter(x=months_p1, y=pct_120_p1, name="120% P1", 
                          line=dict(color='#DC143C', width=2), mode='lines+markers'),
                secondary_y=True
            )
            
            # Procent 120 - P2 (orange)
            fig.add_trace(
                go.Scatter(x=months_p2, y=pct_120_p2, name="120% P2", 
                          line=dict(color='#FF8C00', width=2, dash='dash'), mode='lines+markers'),
                secondary_y=True
            )
            
            # Opdater layout
            fig.update_xaxes(title_text="Måned", tickangle=-45)
        ...
        def create_besøg_line_chart():
            ...
            # P1 (blå)
            fig.add_trace(
                go.Scatter(x=months_p1, y=counts_p1, name="Besøg P1",
                          line=dict(color='#4169E1', width=3), mode='lines+markers')
            )
            
            # P2 (lyseblå)
            fig.add_trace(
                go.Scatter(x=months_p2, y=counts_p2, name="Besøg P2",
                          line=dict(color='#87CEEB', width=3, dash='dash'), mode='lines+markers')
            )
        ...
        def create_uddannelseslæger_line_chart():
            ...
            # Procent uddannelseslæger - P1 (rød)
            fig.add_trace(
                go.Scatter(x=months_p1, y=pct_udd_p1, name="Udd.læger% P1",
                          line=dict(color='#DC143C', width=2), mode='lines+markers'),
                secondary_y=True
            )
            
            # Procent uddannelseslæger - P2 (orange)
            fig.add_trace(
                go.Scatter(x=months_p2, y=pct_udd_p2, name="Udd.læger% P2",
                          line=dict(color='#FF8C00', width=2, dash='dash'), mode='lines+markers'),
                secondary_y=True
            )
