...
def load_data(uploaded_file):
    ...
    df["dato"] = pd.to_datetime(df["ydelses_dato"], unit="d", origin="1899-12-30")
...
def lav_månedsserie(df, koder, label):
    d = df[df["ydelseskode"].isin(koder)]["antal"].sum()
    return d.groupby(["år", "måned"]).size().reset_index(name=label)
...
def ugedagsgraf(df1, df2):
    ...
    def prep(df, year):
        ...
        d = d.groupby("ugedag").size().reset_index(name="antal")
        d["periode"] = year
        return d
